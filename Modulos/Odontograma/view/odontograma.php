...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Suponiendo que 'param' es el par�metro que quieres decodificar
        const encodedParam = params.get('param');

        if (encodedParam) {
            // Decodificar el par�metro en base64
            let parametros = window.atob(encodedParam);

            // Parsear el JSON
            const data = JSON.parse(parametros);

            // Obtener el �ltimo objeto del array `parametros`
...
<script>
    $(document).ready(() => {
        ...
        if(nombrePacient == "" && documentoPaciente == "" && episodio != ""){
            mostrarAlerta("Episodio no encontrado", "No se encontró información para el episodio ingresado.");
            return;
        }

        if(ubicacionPaciente == "" && camaPaciente == "" && episodio != "" && nombrePacient != "" && documentoPaciente != ""){
            mostrarAlerta("Paciente de alta", "El paciente con el episodio asociado está en estado de alta");
            return;
        }
    });
