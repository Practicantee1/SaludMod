$(document).ready(function() {
    $('#guardarFirmaEntrada').on('click', function(event) {
        ...
        // Datos del formulario 'firmas'
        let firmaData = [];
        let hasNullFields = false;

        // Recorre cada fila del formulario para obtener los datos
        $('#firmas-container .firma-item').each(function(index) {
            var episodio = $("#episodio").val();
            // var cargo = $(this).find('select[name="idCargoEntrada[]"]').val();       
            var cargo = $(this).find('input[name="idCargoEntrada[]"]').val();
            var nombreCompleto = $(this).find('input[name="idNombreFirmaEntrada[]"]').val();
            var numeroDocumento = $(this).find('input[name="idDocumentoFirmaEntrada[]"]').val();
        ...
        $.ajax({
            type: 'POST',
            url: '../logica/formFirmaEntradaSQL.php',
            data: {
            ...
            success: function(response) {
                ...
                if (jsonResponse.error) {
                    ...
                    $('#firmas input, #firmas select, #firmas textarea, #firmas button').prop('disabled', true); 
